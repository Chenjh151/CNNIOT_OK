...
overlay = Overlay('cnn_demo.bit')   # load bitstream inside FPGA
...
class Convolution2D:
    ...
    def forward(self, data, dma, Load_Input=1, Load_Weight=1):
        ...
        in_buffer = pynq.allocate(shape=(3000,), dtype=np.int32)
        data = np.multiply(data, self.Precision)
        x = data.flatten()
        size = data.shape

        Configuration = pynq.allocate(shape=(17,), dtype=np.int32)
        Configuration[0] = 0
        ...
        for idx in range(int((x.shape[0]) / 3000)):
            ...
            dma.recvchannel.wait()
        if ((x.shape[0]) % 3000 != 0):
            in_buffer_remind = pynq.allocate(shape=(((x.shape[0]) % 3000),), dtype=np.int32)
        ...
        if (self.Bias.any()):
            ...
            if ((self.Bias.shape[0]) % 3000 != 0):
                in_buffer_remind = pynq.allocate(shape=(((self.Bias.shape[0]) % 3000),), dtype=np.int32)
        ...
        if ((self.Weight.shape[0]) % 3000 != 0):
            in_buffer_remind = pynq.allocate(shape=(((self.Weight.shape[0]) % 3000),), dtype=np.int32)
        ...
        in_buffer_remind = pynq.allocate(shape=(3,), dtype=np.int32)
        dma.sendchannel.transfer(in_buffer_remind)
        dma.sendchannel.wait()
        dma.recvchannel.transfer(in_buffer_remind)
        dma.recvchannel.wait()
        ...
        if ((Total_result) % 3000 != 0):
            in_buffer_remind = pynq.allocate(shape=(((Total_result) % 3000),), dtype=np.int32)
            dma.sendchannel.transfer(in_buffer_remind)
            dma.sendchannel.wait()
            dma.recvchannel.transfer(in_buffer_remind)
            dma.recvchannel.wait()
    ...
    def forward(self, data, dma, Load_Input=1):
        ...
        for i in range(self.Output):
            ...
            for idx in range(int(size / 3000)):
                ...
                dma.recvchannel.wait()
            if ((size) % 3000 != 0):
                in_buffer_remind = pynq.allocate(shape=(((size) % 3000),), dtype=np.int32)
